...
const EMAILJS_TEMPLATE_ID_SAMPLES = import.meta.env.VITE_EMAILJS_TEMPLATE_ID_SAMPLES || 'your_samples_template_id';
...
export const sendSampleRequestEmail = async (formData: SampleRequestData): Promise<void> => {
  try {
    const templateParams = {
      from_name: formData.name,
      from_company: formData.company,
    ...
    console.error('EmailJS Error:', error);
    throw new Error('Failed to send sample request. Please try again or contact us directly.');
  }
};
